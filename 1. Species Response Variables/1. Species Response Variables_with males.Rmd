...
ggplot(wtd_dets, aes(x = array, y = wolf_present / n_weeks))+
  geom_jitter()+
  ylab("Proportion Summer Weeks Present")+
  xlab("Array")+
  theme_classic()
...
library(factoextra)
library(rgl)
#First isolate them and rename them
...
covs<-covs%>%
  ...
  pipeline = pipeline)%>%
  dplyr::select(array, site, disturbance_index, broadleaf, roads, well_gas, well_oil, well_bitumen, inactive_well, seismic, seismic_3D, harvest, pipeline)
